        ...
        <section className={Style.cart}>
          ...
          <footer>
            ...
            <div className={Style.btn_div}>
              <button className={`${Style.btn} ${Style.clearbtn}`} onClick={()=>{dispatch(clearCart({userId , dispatch}))}}>Clear Cart</button>
              <button className={`${Style.btn} ${Style.buybtn}`} onClick={()=>{navigate("/selectAddress" , {state:{entry:true}})}}>Proceed to Buy</button>
            </div>
          </footer>
        </section>
      )
