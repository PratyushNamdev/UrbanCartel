...
export default function AddressForm() {
 ...
 // const { isLoading } = useSelector((state) => state.address);
  ...
  const handleSubmit = async (e) => {
    ...
    //this is for the user submitting the edited form
    if (address._id) {
      ...
      const res = await dispatch(updateAddress({ obj, id: address._id , dispatch}));
    ...
    //for user adding new address
    else {
      //the address is sent as selectedAddress because addAddress async thunk take a single arguement and send the data to server as address.selected address , it is written in this way to avoid collison from the address from used in the addressPicker component
      const res = await dispatch(addAddress({selectedAddress : address , dispatch}));
      ...
      <div>
        <section className={Style.address_form_container}>
          <form onSubmit={handleSubmit} className={Style.address_form}>
            ...
            <button className={Style.address_submit_btn} type="submit">
              Procced
            </button>
          </form>
        </section>
