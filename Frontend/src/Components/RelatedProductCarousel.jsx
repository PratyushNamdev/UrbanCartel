...
export default function RelatedProductCarousel({ sub_category , currentProductId }) {
  const responsive = {
    superLargeDesktop: {
  ...
  const initialCall = async (sub_category) => {
    ...
    try {
      const result = await api.get(
        `/api/product/products?sub_category=${sub_category}`
      );
      if (result.success) {
        setData({
  ...
  const nextCall = async (sub_category) => {
    ...
    try {
      const result = await api.get(
        `/api/product/products?sub_category=${sub_category}&page=${page}`
      );
      if (result.success) {
        setData((prevData) => ({
      ...
      >
        {
          ...
          data.productData.map((data) => {
            if (data.images.length >= 1 && data._id !== currentProductId) {
